...
namespace ToDoList.Utility
{
    public class TaskManager
    {
        ...
        {
            int userCalenderChoice = _inputManager.GetCalenderOption();
            CalenderChoice calenderChoice = (CalenderChoice)userCalenderChoice;
        ...
        private void PrintSameParameterTasks(List<Task> userTask, CalenderChoice calenderChoice)
        {
            switch (calenderChoice)
            {
                case CalenderChoice.Day:
                    foreach (Task task in userTask)
                    {
        ...
        private List<Task> RecurByParameter(List<Task> userTask, Task task, RecurrenceField recurrenceField)
        {
            ...
            {              
                ...
                case RecurrenceField.Monthly:
                    ...
                    break;
                case RecurrenceField.Yearly:
                    while (count < 3)
                    {
                        Task recurrentTask = new Task();
                        recurrentTask.TargetDate = initialDate.AddYears(count);
                        recurrentTask.Heading = task.Heading;
                        recurrentTask.Recurrence = task.Recurrence;
                        recurrentTask.Description = task.Description;
                        userTask.Add(recurrentTask);
