...
namespace TimeTracker.Controller
{
    internal class TaskController
    {
        ...
        private User CalculateTimeExecuted(User user, int taskIndex)
        {
            ...
            if (pausedTimesCount != 0)
            {
                if (pausedTimesCount == user.UserTasks[taskIndex].ResumedTimesList.Count)
                {
                    for(int count = 0; count < pausedTimesCount; count++)
                    {
                        user.UserTasks[taskIndex].TimeExecuted += user.UserTasks[taskIndex].ResumedTimesList[count] - user.UserTasks[taskIndex].PausedTimesList[count];
                    }
                }
            }
        ...
        private void GenerateMonthlySummary(List<UserTask> tasks)
        {
            ...
            consoleTable.AddRow(totalTime, totalTime / (endOfMonth - startOfMonth), mostProductiveDay + " - " + highestTimeSpent, longestTaskDetails, shortestTaskDetails, tasksCount);
            consoleTable.Write();
        }
    }
}
